package io.github.lucaspg.tutorialmod.util;
...
public class RenderUtils {
    ...
    public static void renderLaserBeam(RenderLevelStageEvent event, Player player, float ticks) {
        ...
        Matrix4f positionMatrix = currentPose.pose();

        VertexConsumer builder = buffer.getBuffer(beaconBeam(LASER_BEAM_TEXTURE));
        ...
        float f = (Mth.lerp(ticks, player.xRotO, player.getXRot()) - Mth.lerp(ticks, localPlayer.xBobO, localPlayer.xBob));
        float f1 = (Mth.lerp(ticks, player.yRotO, player.getYRot()) - Mth.lerp(ticks, localPlayer.yBobO, localPlayer.yBob));
        ...
        int alpha = 150;
        renderQuad(builder, positionMatrix, vector3f, vec1, vec2, vec3, vec4, alpha, r, g, b);
        renderQuad(builder, positionMatrix, vector3f, vec5, vec6, vec7, vec8, alpha, r, g, b);
        renderQuad(builder, positionMatrix, vector3f, vec9, vec10, vec11, vec12, alpha, r, g, b);
        renderQuad(builder, positionMatrix, vector3f, vec13, vec14, vec15, vec16, alpha, r, g, b);
    }

    private static void renderQuad(VertexConsumer builder, Matrix4f positionMatrix, Vector3f normalVector, Vector4f vec1, Vector4f vec2, Vector4f vec3, Vector4f vec4, int alpha, int r, int g, int b) {
        builder.addVertex(positionMatrix, vec4.x(), vec4.y(), vec4.z()).setColor(FastColor.ARGB32.color(alpha, r, g, b)).setUv(0, (float) 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(normalVector.x(), normalVector.y(), normalVector.z());
        builder.addVertex(positionMatrix, vec3.x(), vec3.y(), vec3.z()).setColor(FastColor.ARGB32.color(alpha, r, g, b)).setUv(0, (float) 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(normalVector.x(), normalVector.y(), normalVector.z());
        builder.addVertex(positionMatrix, vec2.x(), vec2.y(), vec2.z()).setColor(FastColor.ARGB32.color(alpha, r, g, b)).setUv(0, (float) 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(normalVector.x(), normalVector.y(), normalVector.z());
        builder.addVertex(positionMatrix, vec1.x(), vec1.y(), vec1.z()).setColor(FastColor.ARGB32.color(alpha, r, g, b)).setUv(0, (float) 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(normalVector.x(), normalVector.y(), normalVector.z());
    }

    public static RenderType beaconBeam(ResourceLocation location) {
